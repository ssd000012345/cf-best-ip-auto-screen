    ...
    for url in THIRD_PARTY_URLS:
        try:
            ...
            if "uouin.com" in url:
                ...
                for ip in ipv4 + ipv6:
                    if len(ip) > 6: ips.add(ip.strip())
            else:
                for line in r.text.splitlines():
                    line = line.strip()
                    if line and not line.startswith(('#','//')):
                        ip_part = line.split()[0].split(':')[0] if ':' in line else line
                        if len(ip_part) > 6: ips.add(ip_part)
        except: pass
    return list(ips)
...
def main():
    log("🚀 V2RayN实用版 CF优选IP筛选（数量800 + 速度优先）启动...")
    all_ips = set()
    ...
    test_ips = list(all_ips)[:800]   # ← 按你要求降到800
    ...
    with open("best_cf_ips.txt", "w", encoding="utf-8") as f:
        f.write(f"# V2RayN实用 CF优选IP榜单（速度优先） - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"# 测试数量: 800 | 可用IP: {len(results)}\n\n")
        f.write("=== 综合最优 Top 30（推荐直接复制到V2RayN） ===\n")
        for ip, latency, loss, speed, score, family in top_ips:
            f.write(f"{ip:<18} | {latency:>6.2f}ms | {loss:>5.1f}%丢包 | {speed:>6.2f} MB/s | {family}\n")

    log(f"✅ 完成！输出 {len(top_ips)} 个可用IP（已优化适合V2RayN）")
